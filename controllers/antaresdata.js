...
exports.findAll = (req, res) => {
  ...
  DataMeter.getJumlah((err, data) => {
    ...
    // var jumlah1 = JSON.parse(JSON.stringify(jumlah)).cnt;
  });
};
...
exports.create = async (req, res) => {
  ...
  // DataMeter.getAll((err, data) => {
  //   var namaMeter = data;
  //   console.log("NAMA METER: ", namaMeter);

  //  // var namaMeter1 = JSON.parse(parse(stringify(namaMeter)).nama_meter);

  //   console.log(namaMeter1);
  // });
  ...
  try {
    const response = await axiosInstance.get(
      "~/antares-cse/antares-id/Energy_Meter/90111393_UI-ATM-FMIPA2/la"
    );
  ...
  //var newData = TA73data.UnmarshalTA73(APIData);

  DataAntares.create(newData, (err, data) => {
    if (err)
      res.status(500).send({
        message:
          err.message || "Some error occurred while creating the AntaresData.",
      });
    else res.send(data);
  });
};
